...
function arrayToProposal(input: string[]) {
  let newProposal: Proposal = {
    ...
    type: input[3],
    description: input[4],
    xCord: parseInt(input[5], 10),
    yCord: parseInt(input[6], 10),
    imageLink: input[7] || undefined,
    threadLink: input[8],
    dateProposed: parseSheetDate(input[9]) ?? 0,
    actionDate: parseSheetDate(input[10]),
    objections: parseSheetProposals(input[11]) ?? ["0"],
    numExtensions: parseInt(input[12]),
    otherJson: parseJsonFromSheet(input[13]),
  };
